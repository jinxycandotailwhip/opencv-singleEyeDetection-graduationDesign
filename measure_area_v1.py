...
def watershed_demo(image):
    blackboard = np.ones(image.shape[:2], dtype=np.uint8)
    ...
    blackboard[markers == -1] = 255  #把markers=-1的点都标红（画出分水岭）
    return blackboard
...
def measure_contours(image):
    ...
    for i, contour in enumerate(contours):
        ...
        if bool(mm["m00"]):
            ...
            pass
        cv.drawContours(image, contours, i, 255, 2)
        cv.imshow("contours", image)
        cv.putText(image, "area:%s" % area, (100, 20), cv.FONT_HERSHEY_SIMPLEX, 0.75, 255, 1)
        return image
